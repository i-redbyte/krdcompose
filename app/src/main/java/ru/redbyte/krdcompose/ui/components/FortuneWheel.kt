...
/**
 * Модель элемента колеса.
 *
 * @property text Текст, который будет выведен в секторе. Может быть любой строкой
 * (число, слово, короткая фраза). Рекомендуется удерживать длину текста разумной,
 * чтобы он помещался в сектор при различных размерах колеса.
 */
data class WheelItem(val text: String)

/**
 * Набор функций замедления (easing), доступных для анимации вращения.
 * Ключ — отображаемое имя в UI, значение — функция `f(t)` в диапазоне [0;1].
 * Все функции монотонно возрастают и возвращают 0 при `t=0` и 1 при `t=1` (за редкими оговорками внутри
 * некоторых формул, которые обрабатываются явно).
 *
 * Список кривых:
 * - **Sine‑out** — мягкое синусоидальное замедление.
 * - **Cubic‑out / Quint‑out / Quad‑out / Quart‑out** — полиномиальные ease‑out разной «длины» хвоста.
 * - **Expo‑out** — экспоненциальное, быстро ускоряется и долго затухает.
 * - **Circ‑out** — по дуге окружности, очень плавное.
 * - **Back‑out** — лёгкий «перелёт» за цель и возврат.
 * - **Elastic‑out** — пружинит с затухающими колебаниями.
 * - **Bounce‑out** — подпрыгивания, имитирует отскок.
 * - **Linear** — линейно, без замедления.
 * - **…‑inOut** — симметричные варианты с плавным разгоном и замедлением.
 */
...
/**
 * Компонент «Колесо фортуны».
 *
 * Рисует круг, разбитый на равные сектора, вращается по нажатию, плавно замедляется и останавливается
 * так, чтобы центр выбранного сектора оказался под фиксированной красной стрелкой, направленной вниз.
 * При завершении вызывает [onItemSelected] с фактическим элементом, оказавшимся под стрелкой.
 *
 * Выбор сектора может быть случайным (по умолчанию) или принудительным через [forcedWinnerIndex].
 * Компонент сохраняет состояние вращения между пересозданиями (ориентация, конфигурация).
 *
 * Математика совмещения: прицел — угол 90° (вниз). Каждый сектор имеет центр `90° + index * (360/N)`. Мы
 * вычисляем дельту до ближайшего такого центра, добавляя несколько полных оборотов, и анимируем поворот
 * от текущего угла до рассчитанной целевой позиции.
 *
 * @param items Список элементов, минимум 2. Индекс элемента соответствует индексу сектора по часовой стрелке.
 * @param modifier Внешний модификатор.
 * @param evenSectorColor Цвет секторов с чётным индексом (0, 2, 4, …).
 * @param oddSectorColor Цвет секторов с нечётным индексом (1, 3, 5, …).
 * @param evenTextColor Цвет текста на чётных секторах. Если `null`, подбирается автоматически по контрасту.
 * @param oddTextColor Цвет текста на нечётных секторах. Если `null`, подбирается автоматически по контрасту.
 * @param easing Функция замедления `f(t)`; принимает `t ∈ [0;1]`, возвращает прогресс.
 * @param forcedWinnerIndex Если не `null` и попадает в `items.indices`, вращение всегда завершится на этом секторе.
 * Иначе победитель выбирается случайно при каждом запуске.
 * @param onItemSelected Колбэк с фактическим элементом, который оказался под стрелкой в конце вращения.
 *
 * ### Пример использования
 * ```kotlin
 * val easingName by remember { mutableStateOf("Sine‑out") }
